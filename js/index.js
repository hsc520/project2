<script type="text/javascript">
		$(document).ready(function(){
			...
			$(".banner>ul").css({"width":winW*liLen+"px"});
			...
			$(".banner>ul>li").click(function(){
				...
				if(bool){
					bool = false;
					$(".banner>ul").animate({marginLeft:-winW*index+"px"},function(){bool = true;});
					$(".banner>ul>li").eq(index).addClass("on").siblings().removeClass('on');
				}
			})
			$(".banner>.next").click(function(){
				index = $(".banner>ul>li.on").index();
				...
				if(bool){
					...
					$(".banner>ul").animate({marginLeft:-winW*index+"px"},700,function(){bool = true;});
					$(".banner>ul>li").eq(index).addClass("on").siblings().removeClass('on');
				}
			...
			*/
			function show(){
				...
				lunbo = setInterval(function(){
					...
					/*animate()动画*/
					$(".banner>ul").animate({marginLeft:-winW*index+"px"});
